...
%hVals = logspace( 2, -2, 20 );
%hVals = logspace( 5, 1, 10 );
hVals = logspace( -10, 5, 10 );
%hVals = logspace( -2, 1, 10 );
...
%    f = waitbar(i_Q/num_Q, f, sprintf("Trying with orthogonal matrix #%d", i_Q));
    ...
    %{
    [U,~] = qr(randn(3)); [V,~] = qr(randn(3));     
    A = U*diag([1 1e-6 1e-6])*V';
    %A = U*diag([1 1e-1 1e-1])*V';
    % linear example
    f1 = @(x1,x2,x3) A(1,1)*x1 + A(1,2)*x2 + A(1,3)*x3 + sigma .* (Q(1,1).*x1 + Q(1,2).*x2 + Q(1,3).*x3);
    f2 = @(x1,x2,x3) A(2,1)*x1 + A(2,2)*x2 + A(2,3)*x3 + sigma .* (Q(2,1).*x1 + Q(2,2).*x2 + Q(2,3).*x3);
    f3 = @(x1,x2,x3) A(3,1)*x1 + A(3,2)*x2 + A(3,3)*x3 + sigma .* (Q(3,1).*x1 + Q(3,2).*x2 + Q(3,3).*x3);
    %}
    
    del = 1e-10; 
    % Olly linear example
    f1 = @(x1,x2,x3) (1+del)*x1 + x2 + (1+del)*x3 ;
    f2 = @(x1,x2,x3) (1+del)*x1 + (1+del)*x2 + x3;
    f3 = @(x1,x2,x3) x1 + (1+del)*x2 + (1+del)*x3;
    

    A = [1+del 1 1+del;1+del 1+del 1;1 1+del 1+del];
    %{
    AA = inv(A); 
    ...
    f1 = @(x1,x2,x3) Ainv(1,1)*f1tmp(x1,x2,x3) + Ainv(1,2)*f2tmp(x1,x2,x3) + Ainv(1,3)*f3tmp(x1,x2,x3); 
    f2 = @(x1,x2,x3) Ainv(2,1)*f1tmp(x1,x2,x3) + Ainv(2,2)*f2tmp(x1,x2,x3) + Ainv(2,3)*f3tmp(x1,x2,x3); 
    f3 = @(x1,x2,x3) Ainv(3,1)*f1tmp(x1,x2,x3) + Ainv(3,2)*f2tmp(x1,x2,x3) + Ainv(3,3)*f3tmp(x1,x2,x3); 

        expected = (2*rand(1,3) - 1)/10000;
        p1 = chebfun3(@(x1,x2,x3) f1(x1-expected(1), x2-expected(2), x3-expected(3)),'degree',[2 2 2]);
        p2 = chebfun3(@(x1,x2,x3) f2(x1-expected(1), x2-expected(2), x3-expected(3)),'degree',[2 2 2]);
        p3 = chebfun3(@(x1,x2,x3) f3(x1-expected(1), x2-expected(2), x3-expected(3)),'degree',[2 2 2]);
