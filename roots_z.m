function [rts, R, V, W, approx_err, eig_err] = roots_z(f1,f2,f3,max_degree)
    ...
    [R, n_s1, n_s2, ~, ~, n_z, approx_err] = cayley_resultant(f1,f2,f3,max_degree);
    toc
    ...
    elseif (z_length <= 2)
        [V,D,W] = eig(R(:,:,1),-R(:,:,2));
        rts = diag(D);           % %% ADDED: keep unit-z values
        ...
        for i = 1:m
            vi = V(:,i);
            ri = (R1 + rts(i)*R2) * vi;          % residual in unit coords
        ...
        for i = 1:m
            vi = V(:,i);
            ri = (C2 - rts(i)*C1) * vi;          % residual in unit coords
            nz_eff = z_length;                         % effective # z-slices used
            lin_back_err_all(i) = nz_eff * (norm(ri,2) / max(norm(vi,2), eps));
        end
    end
    ...
    if ~isempty(rts)
        ...
        interval_mask = -1 <= real(rts) & real(rts) <= 1;
        keep = real_mask & interval_mask;
    
        rts = real(rts(keep));                   % returned roots (physical)
        V   = V(:, keep);
        W   = W(:, keep);
